...
<template>
  <nav id="app-navbar" :class="['fixed top-0 left-0 right-0 z-99999 transition-all duration-500', navBg ? 'backdrop-blur-md bg-white/90 border-b border-white/20 shadow-lg scrolled' : 'bg-transparent']" role="navigation" aria-label="Main navigation">
    ...
    <transition name="slide-fade">
      <div v-if="mobileOpen" class="md:hidden bg-[#ffffffb9] shadow-md border-t border-gray-100">
        <div class="px-4 py-4 space-y-2">
          ...
          <router-link to="/mcu" @click="closeMobile" class="block py-2 font-medium relative transition-all duration-300" :class="getMobileNavLinkClass('/mcu')">
            Produk & Layanan
            <span v-if="currentRoute === '/mcu'" class="absolute bottom-1 left-0 w-1 h-6 bg-[#FF7D14] rounded-full"></span>
          </router-link>
