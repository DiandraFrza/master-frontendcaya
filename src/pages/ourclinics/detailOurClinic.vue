    ...
    <main class="page-content">
      ...
      <section class="py-12 bg-slate-50">
        <div class="container mx-auto px-4 max-w-6xl">
          ...
          <div class="max-w-2xl mx-auto mb-12">
            <div class="relative">
              <input v-model="query" type="text" placeholder="Cari berdasarkan nama klinik, alamat, atau kota..." class="w-full px-6 py-4 pl-12 rounded-2xl border border-slate-300 focus:border-[#F46700] focus:ring-2 focus:ring-[#F46700]/20 transition-all duration-300 bg-white shadow-sm" />
          ...
          <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            <div v-for="branch in filtered" :key="branch.id" class="bg-white rounded-2xl shadow-sm hover:shadow-md transition-all duration-300 border border-slate-100 overflow-hidden group flex flex-col h-full">
              <!-- Header dengan Gradient -->
              <div class="bg-linear-to-r from-[#FF7D14] to-[#ff562c] p-6 text-white">
                <div class="flex items-center justify-between mb-2">
                  <h3 class="text-xl font-bold">{{ branch.name }}</h3>
                  <div v-if="branch.id === 'depok'" class="bg-white/20 px-3 py-1 rounded-full text-xs font-medium backdrop-blur-sm">Pusat</div>
                </div>
              ...
              <div class="p-6 flex-1 flex flex-col">
                ...
                <div class="flex items-start gap-3 mb-6">
                  ...
                  <div class="flex-1">
                    ...
                    <a :href="`tel:${branch.phone}`" class="text-sm text-slate-600 hover:text-[#F46700] transition-colors block">
                      {{ branch.phone }}
                    </a>
                  </div>
                </div>
        ...
        <div class="container mx-auto px-4 max-w-6xl">
          ...
          <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
            <div class="h-96 md:h-120 rounded-2xl overflow-hidden">
              <iframe class="w-full h-full" src="https://maps.app.goo.gl/9FVgiz7SgjASmSbPA" width="100%" height="100%" style="border: 0" allowfullscreen="" loading="lazy" referrerpolicy="no-referrer-when-downgrade"></iframe>
            </div>
      ...
      <section class="py-16 bg-linear-to-r from-[#FF7D14] to-[#ff562c]">
        <div class="container mx-auto px-4 max-w-4xl text-center">
          ...
          <p class="text-white/90 text-lg mb-8 max-w-2xl mx-auto">Tim customer service kami siap membantu Anda memilih klinik terdekat dan menjawab pertanyaan seputar layanan MCU</p>
          <div class="flex flex-col sm:flex-row gap-4 justify-center">
            <a :href="`tel:${sitePhone}`" class="bg-white text-[#F46700] px-8 py-4 rounded-lg hover:bg-slate-100 transition-all duration-300 font-semibold text-lg flex items-center justify-center gap-3">
              <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z"></path>
              </svg>
              Hubungi Kami
            </a>
            <a :href="`https://wa.me/${siteWA.replace(/\D/g, '')}`" target="_blank" class="bg-white/20 text-white px-8 py-4 rounded-lg hover:bg-white/30 backdrop-blur-sm transition-all duration-300 font-semibold text-lg flex items-center justify-center gap-3">
...
function waUrl(branch) {
  const text = encodeURIComponent(`Halo Caya, saya mau tanya layanan MCU di cabang ${branch.name} — alamat: ${branch.address}`);
